...
const OPENHANDS_DRIVER_KIND = ProviderDriverKind.make("openhands");
...
/**
 * Keep the model the OpenHands CLI is configured with. Never send this ID to ACP:
 * OpenHands resolves its LLM from `~/.openhands`, and its ACP session advertises no
 * model state, so T3 has nothing to select against.
 */
export const OPENHANDS_DEFAULT_MODEL = "openhands-default";
...
export const DEFAULT_MODEL_BY_PROVIDER: Partial<Record<ProviderDriverKind, string>> = {
  ...
  [OPENHANDS_DRIVER_KIND]: OPENHANDS_DEFAULT_MODEL,
